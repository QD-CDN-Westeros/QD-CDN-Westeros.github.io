...
try {
	var Common = {
		...
		applyImageLoad: function () {
			$('.search-qd-v1-result, .carousel-qd-v1-shelf, .accessories-qd-v1-wrapper, .header-qd-v1-amazing-menu').QD_smartImageLoad({
				sizes: {
					width: '320',
					height: '425'
				}
		...
		applyAmazingMenuMobile: function () {
			...
			$('.header-qd-v1-amazing-menu-mobile').append('<div class="header-qd-v1-close-amazing-menu-mobile"></div>');
		...
		openSearch: function () {
			$('.header-qd-v1-search-trriger').click(function () {
				$('.header-qd-v1-content').find('.header-qd-v1-search').toggleClass('qd-on');
				return false;
			});
		},
		...
		showFooterLinks: function () {
			$('.footer-qd-v1-menu-list > ul > li > em').click(function (e) {
				var $t = $(this);
				$t.toggleClass('qd-is-active');
				$t.next('ul').toggleClass('qd-is-active');
			});
		},
		fixPlaceholderSearch: function () {
			var idSearchFilterP = $('.header-qd-v1-search input[type="text"].fulltext-search-box');
	...
	var Home = {
		...
		applyMosaicCarousel: function () {
			...
			var mbRow = $('.mosaic-categories-qd-v2-wrapper.banner-qd-v1-responsive > .qd-mb-row');

			wrapper.find('.box-banner').first().clone().removeClass('col-xs-6 col-sm-3').insertBefore(wrapper).wrapAll('<div class="highlight-qd-v1-banner visible-xs visible-sm"></div>');
			...
			wrapper.slick('slickRemove', 0);
		},
		applySpecialBanner: function () {
			var wrapper = $('.special-categories-qd-v1-banner > ul');
	...
	var Search = {
		init: function() {
			...
			Home.applySlickSlider();
			Search.shelfLineFix();
			Search.removeFilterParentheses();
		},
		ajaxStop: function() {},
		windowOnload: function() {},
		openFiltersMenu: function () {
			...
			$('.menu-departamento').prepend('<span class="search-qd-v1-navigator-close hide"></span>');
		...
		removeFilterParentheses: function () {
			$('.search-single-navigator .Tamanho a').each(function () {
				var $t = $(this);
				var text = $t.text();
	...
	var Product = {
		...
		qdClickTableMeasures: function () {
			var wrapper = $(".product-qd-v1-sku-selection .specification:contains('Tamanho')");
		...
		qdCallThumbVideo: function () {
			...
			iframe = '<iframe width="100%" height="500px" src="' + $('td.value-field.Video:first').text().replace('?v=', '/').replace('watch', 'embed') + '?wmode=transparent&rel=0&enablejsapi=1&autoplay=0&mute=0"  frameborder="0" allowfullscreen/>'; 
			...
			$(window).on('QD_SPC_beforeThumbSlick', function(e,wrapper){
				$(wrapper).append($('<div><img src="/arquivos/qd-playIco.png"></div>'));
			});
			// Para o carousel não iniciar com a thumb de vídeo
			// $(window).on('QD_SPC_afterSlick', function(e){
			// 	$('.product-qd-v1-image-thumbs div[data-slick-index=1]').click();
			// });
		},
		...
		checkClueProduct: function () {
			var wrapper = $(".product-qd-v1-product-clue");

			if (wrapper.find('.product-qd-v1-product-clue-content > *').length <= 0) {
				wrapper.addClass('hidden');
			}
		},
		checkDescription: function () {
			var wrapper = $(".product-qd-v1-description");

			if (wrapper.find('.productDescription > *').length <= 0) {
				wrapper.addClass('hidden');
			}
		},
		checkSpecification: function () {
			...
			if (wrapper.find('.group > *').length <= 0 || wrapper.find('td.value-field.Video:first')) {
				wrapper.addClass('hidden');
			}
		},
		replyFirstThumbColor: function () {
			var wrapper = $("<ul class='product-qd-v1-color-active'><li><div class='row'><div class='color-in-stock img-responsive'></div></div></li></ul>");

			wrapper.find('.color-in-stock').append('<img src="'+ skuJson.skus[0].image +'" width="100" height="100" title="'+ skuJson.name +'">');
			wrapper.find('.color-in-stock').append('<h3>'+ skuJson.name +'</h3>');

			wrapper.insertAfter('.sku-qd-v1-color-similar .prateleira h2');
		},
