/* Anonymouswill - 05/07/2021 13:10:41 GMT */
(function (g) {
...
var CheckoutConfirmation = {
  ...
  __fillHiddenFields: function () {
    $('#orderId').val($('#order-id').text().replace('#', ''));
    $('#clientName').val($('.cconf-profile .lh-copy p:first-child').text());
  },
  __infoPay: function () {
    try {
      var is_boleto = document.querySelector(
        '#app-container .cconf-payment p > span'
      ).innerText;
      if (!/Boleto|boleto|Promissory/.test(is_boleto)) return;
      var value = document.querySelector(
        '#app-container .cconf-payment p > span > span'
      ).innerText;
      document.documentElement.style.setProperty(
        '--confirmation-info',
        '"Agora, adicione o seu comprovante de atuação no formulário e efetue o pagamento no valor de ' +
          value +
          ', utilizando o código do boleto."'
      );
    } catch (e) {
      setTimeout(CheckoutConfirmation.__infoPay, 1e3);
    }
  },
  __mask: function () {
  ...
  __wrapperForm: function () {
    $('#app-top, #checkout-confirmation-top').wrapAll(
      '<div class="mz-wrapper"></div>'
    );
    function addInfoSpam() {
      let infoSpam = '<p class="info-spam">Verifique sua caixa de SPAM.</p>';
      $('.bg-washed-green p:nth-child(2)').prepend(infoSpam);
    }
    setTimeout(addInfoSpam(), 2e3);
  },
  __form: function () {
    var form = document.querySelector('#mz-checkout-form');
    var listener = function (ev) {
      ev.preventDefault();
      CheckoutConfirmation.__fillHiddenFields();
      var input_s = ev.target.querySelectorAll('input[name]');
      var data = {};
      input_s.forEach(function (input) {
        data[input.name] = input.value;
      });
      CheckoutConfirmation.__sendForm(data).done(function (response) {
        form.reset();
        $('input').val('');
  ...
  __sendForm: function (data) {
    let documentId = localStorage.getItem('comprovante');
    let req = '/api/dataentities/OP/documents';
    if (documentId !== null) {
      req = '/api/dataentities/OP/documents/' + documentId;
    }
    const opts = {
      type: 'PATCH',
      url: req,
